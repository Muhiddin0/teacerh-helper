...
const NextBreadcrumb = ({ homeElement, separator, containerClasses, listClasses, activeClasses, capitalizeLinks }: TBreadCrumbProps) => {
  ...
  return (
    <div className='w-full'>
      <ul className={"flex flex-wrap items-center gap-2 my-4 text-lg"}>
          ...
          pathNames.map((link, index) => {
            ...
            return (
              <React.Fragment key={index}>
                ...
                <li className={"flex items-center " + itemClasses} >
                  <Link className='flex items-center hover:text-primary transition-all ' href={href}>
                    <span>
                      {itemLink}
                    </span>
                  </Link>
